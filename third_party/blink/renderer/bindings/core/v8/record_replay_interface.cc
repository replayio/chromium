...
extern "C" char* V8RecordReplayReadSystemFileContents(
...
void SetupRecordReplayCommands(v8::Isolate* isolate, LocalFrame* localFrame) {
  ...
  if (recordreplay::IsReplaying()) {
...

    {
      ...
      if (strlen(replayScript) != strlen(gReplayScript)) {
        recordreplay::Print("Replay script length mismatch: %zu vs %zu",
                            strlen(replayScript), strlen(gReplayScript));
        CHECK(false);
      }
      if (strcmp(replayScript, gReplayScript) != 0) {
        recordreplay::Print("Replay script contents mismatch");
        CHECK(false);
      }
    }
